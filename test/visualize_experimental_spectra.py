...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Visualize theoretical or matched experimental Sipros HDF5 spectra. "
            "Each plotted PSM shows the theoretical precursor envelope; experimental "
            "records use a fragment mirror plot, while theoretical records show b/y "
            "fragment envelopes directly. In shifted experimental records, one observed "
            "baseline apex supports an envelope and its other isotope-peak intensities "
            "are reconstructed from the target-abundance theoretical shape."
        )
    )
    parser.add_argument(
        "--input",
        default="data/spectra/spectra_C13_001.000Pct.h5",
        help="Sipros spectra HDF5 file, or a directory containing .h5 files.",
    )
    parser.add_argument(
        "--output",
...
def _read_optional_float_dataset(group, name, count, default=None):
    if name not in group:
        return [default] * count
...
def load_hdf5_records(path):
    ...
    with h5py.File(path, "r") as handle:
        ...
        record_abundances = _read_optional_float_dataset(
            handle["records"],
            "sip_abundance_pct",
            len(psm_ids),
            metadata["target_sip_abundance_pct"],
        )
        ...
        for index in range(n_records):
            ...
            records.append(
                SpectraRecord(
                    ...
                    target_sip_abundance_pct=record_abundances[index],
...
def _plot_fragments(ax, record, mz_min=None, mz_max=None):
    ...
    ax.set_xlabel("m/z")
    ax.set_ylabel("Relative intensity")
    ax.set_title("Fragments: Apex-supported Shifted Model vs Theoretical")
    ax.text(0.01, 0.96, "Apex-supported model", transform=ax.transAxes, ha="left", va="top", fontsize=8)
...
def _plot_records(records, input_path, output_path, args):
    ...
    theoretical_only = all(record.record_kind.lower() == "theoretical" for record in plotted)
    figure_kind = "Theoretical Spectra" if theoretical_only else "Matched Experimental Spectra"
    fig.suptitle(
        f"{figure_kind} from {input_path.name} "
